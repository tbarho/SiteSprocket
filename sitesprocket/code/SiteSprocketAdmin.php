...
class SiteSprocketAdmin extends LeftAndMain implements PermissionProvider
{
	...
	static $allowed_actions = array (
		...
		'prevpage',
		'updateperpage',
		'buildproject' => 'SITESPROCKET_BACKEND'
	);
	...
	/**
	 * Run a bash script and spit out a simple web response and code (for AJAX)
	 *
	 * @return SS_HTTPResponse
	 */	
	protected function runbashscript($scriptPath, $paramArray) {
		...
		if (is_resource($process)) {
			...
			fwrite($pipes[0], $scriptPath . $parameters);
			fclose($pipes[0]);
			...
			return new SS_HTTPResponse(sprintf('%s', $message), 200);
		}

	}
	
	/**
	 * A button for calling the build project bash script - see ssp_admin.js for AJAX implementation
	 *
	 * @return InlineFormAction
	 */
	public function BuildProjectButton() {
		$a = new InlineFormAction('BuildProject','Build Project');
		return $a;
		
	}
	
	/**
	 * Builds a new project using a bash script
	 *
	 * @return SS_HTTPResponse
	 */	
	public function buildproject() {
		...
		if(is_numeric($this->getRequest()->requestVar('id'))) {
			...
			$project_id = $this->getRequest()->requestVar('id');
			
			return $this->runbashscript('/Users/tybarho/Sites/Scripts/check_mysqlDB_exist.sh', array($project_id, 'root'));
